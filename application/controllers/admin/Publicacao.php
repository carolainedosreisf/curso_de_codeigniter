...
class Publicacao extends CI_Controller {
	...
	public function index($pular=null,$post_por_pagina=null){
		...
		$config['base_url'] = base_url("index.php/admin/publicacao");//arrumar essa rota
		...
		$dados['publicacoes'] = $this->modelpublicacao->listar_publicacao($pular,$post_por_pagina);//$pular,$post_por_pagina
	...
	public function inserir(){
		...
		$this->form_validation->set_rules('txt-subtitulo', 'Subítulo',
		'required|min_length[3]');
		...
		if($this->form_validation->run() == FALSE){
			...
			if($this->modelpublicacao->adicionar($titulo,$subtitulo,$conteudo, $datapub,$categoria,$userpub)){
				redirect(base_url('index.php/admin/publicacao'));//arrumar essa rota
			}else{
	...
	public function salvar_alteracoes($idCrip){
		...
		if ($this->form_validation->run() == FALSE){
			...
			if($this->modelpublicacao->alterar($titulo,$subtitulo,$conteudo,$datapub,$categoria,$id)){
				redirect(base_url('index.php/admin/publicacao'));
			}else{
				echo "Houve um erro no sistema!";
			}
		}
	...
	public function nova_foto(){
		...
		if(!$this->upload->do_upload()){
			...
			if($this->image_lib->resize()){
				...
				redirect(base_url('index.php/admin/publicacao/alterar/'.$id));//arrumar essa rota
				}else{
					echo "Houve um erro no sistema!";
				}
